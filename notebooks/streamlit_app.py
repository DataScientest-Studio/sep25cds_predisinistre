...
def load_artifacts():
    ...
    """
    required = ['best_pipeline.pkl', 'cat_modalities.pkl',
                 'model_metadata.json', 'num_stats.json']
...
def main():
    ...
    with st.sidebar:
        ...
        <div class="sidebar-info">
            <b>Modèle :</b> {metadata['model_name']}<br>
            <b>ROC-AUC :</b> {metadata['roc_auc_test']:.3f}<br>
            <b>F1 (fraude) :</b> {metadata['f1_test']:.3f}<br>
            <b>Recall :</b> {metadata['recall_test']:.3f}<br>
            <b>Précision :</b> {metadata['precision_test']:.3f}<br>
            <b>Seuil optimal :</b> {metadata['optimal_threshold']:.2f}
        </div>
        """, unsafe_allow_html=True)
        st.divider()
    ...
    with tab2:
        ...
        for col, label, val, css_class, desc in [
            ...
            (m2, "F1-Score (fraude)", metadata['f1_test'],        "metric-card-f1",       "Équilibre précision/recall"),
            (m3, "Recall (fraude)",   metadata['recall_test'],    "metric-card-recall",   "Fraudes détectées / total"),
            (m4, "Précision",         metadata['precision_test'], "metric-card-precision","Alertes vraiment frauduleuses"),
        ]:
            with col:
                st.markdown(f"""
